...
export const trackAction = async (userId, userName, userEmail, actionType, metadata = {}) => {
  try {
    // Safety check: ensure db is initialized (client-side only)
    if (!db || typeof window === 'undefined') {
      console.warn("⚠️ Analytics skipped: Not in browser or db not initialized");
...
export const trackUserRegistration = async (userData) => {
  try {
    // Safety check: ensure db is initialized (client-side only)
    if (!db || typeof window === 'undefined') {
      console.warn("⚠️ User registration tracking skipped: Not in browser");
      return;
    }
...
export const getLoanBehaviorAnalysis = async () => {
  try {
    ...
    console.error("Loan Behavior Analysis Error:", error);
    return {
      totalPrompts: 0,
      totalAccepted: 0,
      totalDeclined: 0,
